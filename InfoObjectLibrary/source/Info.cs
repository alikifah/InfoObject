...
{
    public abstract class Info: IEnumerable
    {
        private int size = 0; // data size that this class holds(in Byte)
        internal string signature { get; set; }
        public Info()
        {
            signature = ReflectionProvider.GetSignature(this);
        }
        public int GetSize()
        {
            return size;
        ...
        {
            var data = Serialize(out Exception exception, password);
            if (data != null)
                IOProvider.BinaryToFile(data, filePath);
        }
        public void Save(string filePath, out Exception exception)
        {
            var data = Serialize(out exception, "");
            if (data != null)
                IOProvider.BinaryToFile(data, filePath);
        }
        public void Save(string filePath)
        {
            var data = Serialize(out Exception exception, "");
            if (data != null)
                IOProvider.BinaryToFile(data, filePath);
        }
        ...
        {
            return IOProvider.GetContent(this);
        }
        public int GetContentCount()
        {
            return IOProvider.GetContentCount(this);
        }
