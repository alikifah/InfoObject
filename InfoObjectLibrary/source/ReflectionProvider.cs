...
namespace InfoObject
{
    internal static class ReflectionProvider
    {
        ...
        {
            PropertyInfo[] properties = infoclass.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance).OrderBy(x => x.Name).ToArray();
            return properties;
        }

        internal static FieldInfo[] GetFieldMembers(object structure)
        {
            FieldInfo[] members = structure.GetType().GetFields(BindingFlags.Public | BindingFlags.Instance).OrderBy(x=> x.Name).ToArray();
            return members;
        }
